	...
	>
		...
		{#if hasRightAxis}
			...
			{#each rightTicks as tick, i (i)}
				...
				<text
					x={PAD_LEFT + innerW + 6}
					{y}
					class="{styles['tick-label']} tick-label--right"
					text-anchor="start"
					dominant-baseline="middle"
				>
					{tick.toFixed(dec)}
		...
		{#if hoveredX !== null && !hoveredTickShown}
			...
			<text
				{x}
				y={xAxisY + 10}
				class="{styles['tick-label']} {styles['tick-label--hover']}"
				text-anchor="end"
				transform="rotate(-40, {x}, {xAxisY + 10})"
			>
		...
		{#if hoveredX !== null}
			...
			<rect
				...
				height={26}
				rx={4}
				class={styles['tooltip-bg']}
			/>
			<text
				{x}
				y={firstY - 18}
				class={styles['tooltip-text']}
