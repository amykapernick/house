...
<div class="{styles['chart-wrap']} {className}">
	<svg viewBox="0 0 {W} {H}" class={styles.chart}>
		{#if orientation === 'vertical'}
			<line x1={PAD_LEFT} y1={PAD_TOP} x2={PAD_LEFT} y2={baselineY} class={styles.axis} />
			<line x1={PAD_LEFT} y1={baselineY} x2={PAD_LEFT + innerW} y2={baselineY} class={styles.axis} />
			{#each valueTicks as tick, i (i)}
				{@const y = baselineY - (tick / maxValue) * innerH}
				<line x1={PAD_LEFT} y1={y} x2={PAD_LEFT + innerW} y2={y} class={styles.gridline} />
				<text x={PAD_LEFT - 6} {y} class={styles['tick-label']} text-anchor="end" dominant-baseline="middle">{formatValue(tick)}</text>
			{/each}
		{:else}
			<line x1={PAD_LEFT} y1={PAD_TOP} x2={PAD_LEFT} y2={PAD_TOP + innerH} class={styles.axis} />
			<line x1={PAD_LEFT} y1={PAD_TOP + innerH} x2={PAD_LEFT + innerW} y2={PAD_TOP + innerH} class={styles.axis} />
			{#each valueTicks as tick, i (i)}
				{@const x = PAD_LEFT + (tick / maxValue) * innerW}
				<line x1={x} y1={PAD_TOP} x2={x} y2={PAD_TOP + innerH} class={styles.gridline} />
				<text x={x} y={PAD_TOP + innerH + 14} class={styles['tick-label']} text-anchor="middle">{formatValue(tick)}</text>
			{/each}
		{/if}
		...
		{#each groups as group, gi (group.label)}
			{#if orientation === 'vertical'}
				...
				<text
					...
					class={styles['tick-label']} text-anchor="end"
				...
				{#each group.bars as bar, bi (bar.name)}
					...
					{#if hoveredKey === key}
						...
						<rect x={cx - tipW / 2} y={y - 28} width={tipW} height={22} rx={4} class={styles['tooltip-bg']} />
						<text x={cx} y={y - 13} class={styles['tooltip-text']} text-anchor="middle">{tipText}</text>
					{/if}
				{/each}
			{:else}
				{@const slotY = PAD_TOP + gi * slotSize}
				<text x={PAD_LEFT - 8} y={slotY + slotSize / 2} class={styles['tick-label']} text-anchor="end" dominant-baseline="middle">
					{group.label}
				</text>
				{#each group.bars as bar, bi (bar.name)}
					...
					{#if hoveredKey === key}
						{@const tipText = `${group.label} · ${bar.name}: ${formatValue(bar.value)}${bar.flag ? ` (${bar.flagLabel ?? 'over budget'})` : ''}`}
						{@const tipW = Math.max(70, tipText.length * 6)}
						<rect x={PAD_LEFT + w + 6} y={y + barThickness / 2 - 11} width={tipW} height={22} rx={4} class={styles['tooltip-bg']} />
						<text x={PAD_LEFT + w + 6 + tipW / 2} y={y + barThickness / 2 + 4} class={styles['tooltip-text']} text-anchor="middle">{tipText}</text>
					{/if}
				{/each}
			{/if}
		{/each}
	...
	{#if seriesNames.length > 1}
		<ul class={styles.legend}>
			{#each seriesNames as [name, colour] (name)}
				<li>
					<span class={styles.swatch} style="background: var(--{colour});"></span>
					<span>{name}</span>
				</li>
			{/each}
		</ul>
