<script lang="ts">
	...
	import ContentIcon from '../ContentIcon/index.svelte';
...
>
	...
	{#if quickAddMatch}
		...
		<div class={styles['quick-add']}>
			<p class={styles['quick-add-hint']}>{labels.hint}</p>
			{#if quickAddSuccess}<p class="{styles['quick-add-status']} {styles.success}">{quickAddSuccess}</p>{/if}
			{#if quickAddError}<p class="{styles['quick-add-status']} {styles.error}">{quickAddError}</p>{/if}
		</div>
	{:else}
		<ul
		...
		>
			{#each results as result, i (result.key)}
				<li>
					...
					<a
						class={[styles.result, result.archived && styles.archived]}
						href={result.link}
					...
					>
						...
						{:else if result.Icon}
							<result.Icon />
						{/if}
						<span class={styles.label}>{result.label}</span>
						{#if result.sublabel}<span class={styles.sublabel}>{result.sublabel}</span>{/if}
						{#if result.archived}<span class="{styles.tag} {styles['tag-archived']}">Archived</span>{/if}
						<span class={styles.tag}>{result.section}</span>
					</a>
					<!-- eslint-enable svelte/no-navigation-without-resolve -->
				</li>
			{/each}
			{#if recipeSearchTerm.length >= RECIPE_MIN_CHARS && recipesLoading && recipeResults.length === 0}
				<li class={styles.hint}>Searching recipes...</li>
			{/if}
			{#if results.length === 0 && !recipesLoading}
				<li class={styles.hint}>
					{#if deepSearchMatch}
	...
	<footer class={styles.footer}>
		...
		{:else}
			...
			{#if !term && !deepSearchMatch}
				<span class={styles['quick-add-tip']}><kbd>/task</kbd> <kbd>/shop</kbd> quick add · <kbd>/s</kbd> search everything</span>
			{:else}
				{#if deepSearchMatch?.scope}
					<span class={styles['quick-add-tip']}>searching {deepSearchMatch.scope}</span>
				{/if}
				{#if viewAllHref}
