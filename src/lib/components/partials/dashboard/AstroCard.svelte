...
<div class="astro-card {className}">
	{#if !sun}
		<p class="empty">No location configured in Home Assistant to compute astro times from.</p>
	{:else}
		<div class="times-top">
			<div>
				<p class="label">Sunrise</p>
				<p class="value">{formatTime(sun.sunrise)}</p>
			</div>
			<div>
				<p class="label">Sunset</p>
				<p class="value">{formatTime(sun.sunset)}</p>
			</div>
		</div>

		<svg viewBox="0 0 {W} {H}" class="chart" role="img" aria-label="Sun path from sunrise to sunset - currently {sun.sunElevation >= 0 ? 'above' : 'below'} the horizon at {(sun.sunElevation ?? 0).toFixed(0)} degrees, moon at {(sun.moonElevation ?? 0).toFixed(0)} degrees">
			<line x1="0" y1={CY} x2={W} y2={CY} class="horizon" />
			{#if chart?.dawnX != null}<line x1={chart.dawnX} y1="10" x2={chart.dawnX} y2={CY} class="guide" />{/if}
			{#if chart?.duskX != null}<line x1={chart.duskX} y1="10" x2={chart.duskX} y2={CY} class="guide" />{/if}

			{#if nightBeforePoly}<polygon points={nightBeforePoly} class="night-fill" />{/if}
			{#if dayPoly}<polygon points={dayPoly} class="day-fill" />{/if}
			{#if nightAfterPoly}<polygon points={nightAfterPoly} class="night-fill" />{/if}

			{#if curveLine}<polyline points={curveLine} class="curve" fill="none" />{/if}

			{#if chart}<circle cx={chart.nowX} cy={toY(sun.sunElevation)} r="9" class="sun-marker" />{/if}
			{#if moon}
				<text
					x={moon.x} y={moon.y}
					class="moon-marker" class:below-horizon={!moon.aboveHorizon}
		...
		<div class="times-row">
			<div>
				<p class="label">Dawn</p>
				<p class="value">{formatTime(sun.dawn)}</p>
			</div>
			<div>
				<p class="label">Solar noon</p>
				<p class="value">{formatTime(sun.solarNoon)}</p>
			</div>
			<div>
				<p class="label">Dusk</p>
				<p class="value">{formatTime(sun.dusk)}</p>
			</div>
		</div>

		<div class="times-row">
			<div>
				<p class="label">Moonrise</p>
				<p class="value">{formatTime(sun.moonrise)}</p>
			</div>
			<div>
				<span class="moon-phase-icon" style={sun.southernHemisphere ? 'transform: rotate(180deg);' : undefined} aria-hidden="true">{sun.moonPhase ? MOON_PHASE_ICONS[sun.moonPhase] : '🌙'}</span>
				<p class="value">{sun.moonPhase ? (MOON_PHASE_LABELS[sun.moonPhase] ?? sun.moonPhase) : '—'}</p>
			</div>
			<div>
				<p class="label">Moonset</p>
				<p class="value">{formatTime(sun.moonset)}</p>
			</div>
		</div>
	{/if}
</div>
