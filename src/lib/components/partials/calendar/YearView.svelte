				...
				<div class="day-grid">
					{#each month.weeks as week, weekIndex (weekIndex)}
						{#each week as cell, dayIndex (dayIndex)}
							<div class="day-cell" class:today={cell.isToday} class:outside={!cell.date}>
								{#if cell.date}
...
<style>
	/* TODO: Style the year view - currently bare structural markup only
	   (month/weekday/day-cell/event grid). */
