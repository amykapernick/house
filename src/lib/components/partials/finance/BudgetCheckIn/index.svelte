			...
			{#each rows as row (row.id)}
				<tr>
					...
					<td class={styles.actions}>
						...
						{#if saved.get(row.id) && !saving.get(row.id)}<span class={styles.saved}>Saved</span>{/if}
						{#if errors.get(row.id)}<span class={styles.error}>{errors.get(row.id)}</span>{/if}
					</td>
				</tr>
			{/each}
		</tbody>
