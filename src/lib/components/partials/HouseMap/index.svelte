...
<div class="{styles.container} {className}" style="
...
--width: {size[0]}; --height: {size[1]}" bind:this={containerEl}>
	...
	<svg class={styles.map} viewBox="0 0 {size.join(' ')}" fill="none" bind:this={svgEl} onclick={handleMapClick}>
		{#each areas as area (area.id)}
			<g class={styles.area} style="
...
--colour: var(--{area.colour ?? 'primary'})">
				...
				<a href={editMode ? undefined : area.link} target={editMode ? undefined : '_blank'}>
					...
					<!-- svelte-ignore a11y_no_static_element_interactions -->
					<rect
						class={[styles.space, editMode && styles.draggable]}
						rx="0"
						ry="0"
						x={area.start[0]}
						y={area.start[1]}
					...
					/>
					<text class={styles.label} x={area.start[0] + 10} y={area.start[1] + 40}>
						{area.name}
					</text>
				</a>
				{#if editMode}
	...
	{#each items as item (item.id)}
		...
		<a
			class={[styles.item, editMode && styles.draggable]}
...
--width: {(itemSize[0] / size[0]) * 100}%; --height: {(itemSize[1] / size[1]) * 100}%; --offset_x: {(item.start[0] / size[0]) * 100}%; --offset_y: {(item.start[1] / size[1]) * 100}%; --rotate: {item.rotation ? `${item.rotation}deg` : '0deg'}"
			...
			onpointercancel={() => (dragState = null)}
		>
			{#if Icon}<Icon class={styles['item-icon']} data-power={power} />{/if}
